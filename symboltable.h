...
char keyword[][1000] = {"void","int","double","bool","string","class","interface","null","this","extends","implements","for","while","if","else","return","break","New","NewArray","Print","ReadInteger","ReadLine"};
...
int isIdentifier(char *s){
	...
	for(i=1;i<l;i++)
		flag &= (s[0]>='a' && s[0]<='z') || (s[0]>='A' && s[0]<='Z') || (s[0]=='_') || (s[i]>='0' && s[i]<='9');
	return flag;
}
...
void generateSymbolTable(char *filename){
	//Definitions
	line inp[100];
	char c;
	int lines=0;
	...
	while(i<lines){
		...
		getFirstWord(inp[i].content + strlen(str),name);
		printf("content: %s\t\t\tFirst: %s\t\t\tSecond: %s\n",inp[i].content,str,name);
